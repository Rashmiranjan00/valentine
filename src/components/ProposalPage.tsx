...
export default function ProposalPage({ onYes }: ProposalPageProps) {
  ...
  const moveButton = () => {
    const x = Math.random() * (window.innerWidth - 200) // Adjusted for button width
        ...
        <div className="flex flex-col items-center justify-center gap-8 md:gap-10 p-6 md:p-12 text-center rounded-3xl bg-white/5 dark:bg-white/5 backdrop-blur-sm border border-slate-200 dark:border-white/10 shadow-xl">
          ...
          <div className="relative group">
            ...
            <div className="relative flex items-center justify-center size-24 md:size-32 rounded-full bg-gradient-to-br from-primary/10 to-primary/5 border border-primary/20 shadow-inner">
              <span className="material-symbols-outlined text-6xl md:text-7xl text-primary drop-shadow-sm transition-transform duration-500 group-hover:scale-110">volunteer_activism</span>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3 md:gap-4 max-w-md">
            ...
            <p className="text-slate-500 dark:text-slate-300 text-sm md:text-lg font-light leading-relaxed">
              I've been waiting for the right moment to ask you this. <br className="hidden sm:block"/>Please make me the happiest person alive.
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full justify-center items-center mt-2">
            ...
            >
              <span className="material-symbols-outlined text-[20px] group-hover:hidden">close</span>
              <span className="material-symbols-outlined text-[20px] hidden group-hover:block">sentiment_dissatisfied</span>
              <span>No</span>
            </button>
          </div>
